...
- **Portfolio engine:** Cost basis calculation in `portfolio/services.py` supports FIFO, LIFO (`_process_lot_based(user, lifo)`) and WAC (`_process_wac(user)`), dispatched via `_process_transactions(user, method)`. Separate `fiscal_cost_method` setting for tax reports. All service functions receive `user` as first argument.

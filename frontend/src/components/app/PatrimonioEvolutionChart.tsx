...
// Month keys are "YYYY-MM" strings — lexicographic comparison works correctly.
function filterByRange(data: PatrimonioPoint[], range: Range): PatrimonioPoint[] {
          ...
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 font-mono text-xs tabular-nums">
              <span className="text-muted-foreground">Total&nbsp;
                <span className={totalChg.positive ? 'money-positive' : 'money-negative'}>
                  {totalChg.positive ? '+' : ''}{formatMoney(totalChg.abs)}&nbsp;
                  ({totalChg.positive ? '+' : ''}{totalChg.pct.toFixed(2)}%)
                </span>
              </span>
              <span className="text-muted-foreground">Efectivo&nbsp;
                <span className={cashChg.positive ? 'money-positive' : 'money-negative'}>
                  {cashChg.positive ? '+' : ''}{formatMoney(cashChg.abs)}&nbsp;
                  ({cashChg.positive ? '+' : ''}{cashChg.pct.toFixed(2)}%)
                </span>
              </span>
            </div>
          </div>

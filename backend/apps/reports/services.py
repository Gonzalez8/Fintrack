...
def rv_evolution(user):
    """Return portfolio value time series from PortfolioSnapshot records for `user`."""
...
def patrimonio_evolution(user):
    """Return monthly patrimony evolution for `user` using PortfolioSnapshot + AccountSnapshot.
...
def monthly_savings(user, start_date=None, end_date=None):
    """Return monthly cash balance, investment cost basis, and real savings for `user`.

    Real savings = ΔEfectivo + ΔCosteInv — accounts for cash that was invested.
    Optionally filtered by start_date / end_date (YYYY-MM strings).
    """
    ...
    # --- Investment cost basis: incremental, respects fiscal cost method ---
    settings = Settings.load(user)
    fiscal_method = settings.fiscal_cost_method
    use_wac = fiscal_method == "WAC"
    use_lifo = fiscal_method == "LIFO"
    lots = {}           # FIFO/LIFO: asset_id -> deque of {qty, ppu}
    wac_state = {}      # WAC:  asset_id -> {total_qty, total_cost}
